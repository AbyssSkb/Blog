...
$$
称 $\int\limits_C \overrightarrow{A}(P) d\overrightarrow{\Omega} = \int\limits_C Pdx + Qdy + Rdz$ 为 $\overrightarrow{A}(x,y,z)$ 在 $C$ 上给定方向的**第二型曲线积分**。
...
$$
\begin{split}
    \int\limits_C Pdx + Qdy + Rdz & = \int_a^b Pdx(t) + Qdy(t) + Rdz(t) \\
...
$$
\oint\limits_C P(x,y)dx + Q(x,y)dy = \oiint\limits_D \left(\frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y}\right) dxdy
$$
...
$$
\begin{split}
    \oint\limits_C Pdx + Qdy + Rdz & =\iint\limits_S \left(\frac{\partial R}{\partial y} - \frac{\partial Q}{\partial z}\right) dydz + \left(\frac{\partial P}{\partial z} - \frac{\partial R}{\partial x}\right) dzdx + \left(\frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y}\right) dxdy \\
...
1. $D$ 上任意闭曲线 $C$，有 $\oint\limits_C Pdx + Qdy = 0$
2. $D$ 上第二型曲线积分 $\int\limits_{AB} Pdx + Qdy$ 与路径无关
3. $Pdx + Qdy$ 是全微分
4. $\frac{\partial P}{\partial y} = \frac{\partial Q}{\partial x}$

## 全微分方程
### 定义
如果一阶微分方程 $Pdx + Qdy = 0$ 满足 $\frac{\partial P}{\partial y} = \frac{\partial Q}{\partial x}$，则称此方程为**全微分方程（恰当方程）**。
...
1. $\Omega$ 上任意闭曲线 $C$，有 $\oint\limits_C Pdx + Qdy + Rdz = 0$
2. $\Omega$ 上第二型曲线积分 $\int_{AB} Pdx + Qdy + Rdz$ 与路径无关
3. $Pdx + Qdy + Rdz$ 是全微分
...
$$
d\Omega \cos\alpha_P = dydz,\quad d\Omega\cos\beta_P = dzdx,\quad d\Omega\cos \gamma_P = dxdy
$$
称 $\int\limits_S \overrightarrow{A}(P) d\overrightarrow{\Omega} = \int\limits_S Pdydz + Qdzdx + Rdxdy$ 为 $\overrightarrow{A}(x,y,z)$ 在 $S$ 上给定一侧的**第二型曲面积分**。
...
$$
\begin{split}
    \iint\limits_S Pdydz + Qdzdx + Rdxdy & =\iint\limits_S \left[P\left(-\frac{\partial z}{\partial x}\right) + Q\left(-\frac{\partial z}{\partial y}\right) + R\right]dxdy \\
    & = \pm \iint\limits_D \left[P\left(-\frac{\partial z}{\partial x}\right) + Q\left(-\frac{\partial z}{\partial y}\right) + R\right]dxdy
\end{split}
$$
...
$$
\oiint\limits_S Pdydz + Qdzdx + Rdxdy = \iiint\limits_\Omega \left(\frac{\partial P}{\partial x} + \frac{\partial Q}{\partial y} + \frac{\partial R}{\partial z}\right) d\Omega
$$
...
### 计算
$$
\lim\limits_{\Delta S \to M}\frac{1}{\Delta S} \oint\limits_{\Delta C} Pdx + Qdy + Rdz = \begin{vmatrix}
        \cos\alpha & \cos\beta & \cos\gamma \\
        \frac{\partial}{\partial x} & \frac{\partial}{\partial y} & \frac{\partial}{\partial z} \\
        P & Q & R
    \end{vmatrix}_M
